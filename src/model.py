...
# Communism
os.environ["HF_HOME"] = os.environ.get("HF_HOME", "/home/simin/.huggingface/")
# 这一行设置了环境变量HF_HOME的值。它使用os.environ.get函数检索HF_HOME环境变量的值，如果存在，则使用该值；如果不存在，则将其设置为"/JawTitan/huggingface/"。

import openai
# TestKey: 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
...
class OpenAIDecoder(DecoderBase):
    def __init__(
        self, device, name: str, batch_size: int = 1, temperature: float = 0.8
    ) -> None:
        super().__init__(name, batch_size, temperature, device=device)
        # openai.api_key = os.environ.get("OPENAI_API_KEY", "dummy")
        openai.api_key = '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'
...
class HFTorchDecoder(DecoderBase):
    def __init__(self, device, name: str, batch_size: int = 1, temperature: float = 0.8):
        super().__init__(name=name, batch_size=batch_size, temperature=temperature, device=device)
...
class FsChatDecoder(HFTorchDecoder):
    def __init__(self, device, name: str, batch_size: int = 1, temperature: float = 0.8):
        DecoderBase.__init__(
            self, name=name, batch_size=batch_size,
...
def make_model(device, name: str, batch_size: int = 1, temperature: float = 0.8):
    if name == "codegen-2b":
