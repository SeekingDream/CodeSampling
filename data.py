...
class HumanEvalDataset(object):
    def __init__(
        ...
        self.path = path
        self.data = dict()
        self.raw_data = self.read_problems(path)
        self.mode = mode
        if assertion_path != "":
            self.assertion_data = self.read_problems(assertion_path)
        else:
            self.assertion_data = self.raw_data

        self.data["test"] = self.extract_data()

    def read_problems(self, evalset_file: str) -> Dict[str, Dict]:
        ...
        for pid, prob in self.raw_data.items():
            ...
            if self.mode != "prompt_only":
                ...
                if self.mode == "assertion":
                    ...
                    info_data = assertions[0]
                else:
                    self.raw_data[pid]["assertion"] = assertions
                    info_data = assertions
            else:
                info_data = []
            ...
            new_problem_list.append(new_prob)
        return list(zip(nls, codes, info)), new_problem_list
