...
# from hyx_process_method import get_trace_embedding

import fasttext
from huggingface_hub import hf_hub_download

model_path = hf_hub_download(repo_id="facebook/fasttext-language-identification", filename="model.bin")
emb_model = fasttext.load_model(model_path)
...
class IntraMultiSampleSelector(MultiSampleSelector):
    ...
    def select(
        ...
        for i in range(n_examples):
            max_key = None
            selected_item = None
            if quantile_threshold is not None:  # 这是啥
                filtered_ids = []
                all_second_key = []
                for idx in ids:
                    selected_item = None
                ...
                filtered_ids = ids
            #############################################
            # 双重循环，计算intra-score(投票)
            for idx in filtered_ids:
                item = self.data[idx][i]
                # print(item) # 所有的数据（整个字典）都有
                first_keys = list()
                # second_keys = list()
                for grndtruth_idx in filtered_ids:
                    ...
                    # print("------------item:")
                    # print(item)
                    # print("------------grndtruth_item:")
                    # print(grndtruth_item)
                    # key_extractor = mbr_function
                    key = key_extractor(item, grndtruth_item)  # 每个item都有机会当grndtruth_item !!!差不多！！只要把key_extractor换了就行！
                    # print(key) # 返回True/False
                    first_keys.append(key)

                    # key2 = second_key_extractor(item, grndtruth_item) #每个item都有机会当grndtruth_item !!!差不多！！只要把key_extractor换了就行！
                    # print(key) # 返回True/False
                    # second_keys.append(key2*key)
                first_key = sum(first_keys)
                # second_key = sum(second_keys)
                second_key = (
                    second_key_extractor(item)
                    if second_key_extractor is not None
                    else 0
                ...
                #     # print(grndtruth_item)
                #     # key_extractor = mbr_function
                #     key = second_key_extractor(item, grndtruth_item) #每个item都有机会当grndtruth_item !!!差不多！！只要把key_extractor换了就行！
                #     # print(key) # 返回True/False
...
def load_execution(data_dict, paths, split, tag):
    ...
    if "spider" in paths[0]:
        ...
        exec_list = [
            ...
            # ("trace_embedding.pkl", "trace_embedding"),
            # ("graph_embedding.pkl", "graph_embedding"),
...
def cosine_similarity(emb_x, emb_y):
    ...
    # print(type(emb_x))
    # print(type(emb_y))
    if emb_x is None or len(emb_x) == 0:
        return 0
    if emb_y is None or len(emb_y) == 0:
        return 0
    return np.dot(emb_x, emb_y) / (np.linalg.norm(emb_x) * np.linalg.norm(emb_y))


# def trace_based_function(x, y):
#     if not x["not_degenerate"] or not y["not_degenerate"]:
#         return 0
#     if x["trace_embedding"]==None or y["trace_embedding"]==None:
#         return 0
#     # print(x["trace_embedding"])
#     # print(y["trace_embedding"])
#     emb_x, emb_y = x["trace_embedding"][-1], y["trace_embedding"][-1]
#     return cosine_similarity(emb_x, emb_y)
def process_trace(raw_trace):
    ...
    for line in lines:
        ...
        if 'var' in line and regex.search(pattern, line) is not None:
            # print(regex.search(pattern, line).group())
            trace.append(regex.search(pattern, line).group())
    return trace


def get_trace_embedding(trace):
    # TODO
    import fasttext
    from huggingface_hub import hf_hub_download
    # from gensim.models import Word2Vec

    # a1 = model.get_word_vector("Hello, world!")
    # a2 = model.get_word_vector("Hello, worrd!")
    trace_str = ''
    for t in trace:
        trace_str += t
        trace_str += ', '
    trace_embedding = emb_model.get_word_vector(trace_str)  # len=256
...
def trace_based_function(x, y):
    if not x["not_degenerate"] or not y["not_degenerate"]:
        return 0
    if x["raw_trace"] == None or y["raw_trace"] == None:
        return 0
    # print(x["trace_embedding"])
    # print(y["trace_embedding"])

    # 还可以考虑处理多个
    raw_trace_x, raw_trace_y = x["raw_trace"][-1], y["raw_trace"][-1]
    if raw_trace_x[0] == 1 or raw_trace_y[0] == 1:
        return 0
    if raw_trace_x[-1] is None or raw_trace_y[-1] is None:
        return 0
    trace_x, trace_y = process_trace(raw_trace_x), process_trace(raw_trace_y)
    emb_x, emb_y = get_trace_embedding(trace_x), get_trace_embedding(trace_y)
    return cosine_similarity(emb_x, emb_y)
...
#     if not x["not_degenerate"] or not y["not_degenerate"]:
#         return 0
#     if x["trace"] is None or y["trace"] is None:
#         return 0

#     ### 在这处理


def graph_embedding_based_function(x, y):
    if not x["not_degenerate"] or not y["not_degenerate"]:
        return 0
    # if x["trace_embedding"] is None or y["trace_embedding"] is None:
    #     return 0
    # print(x["trace_embedding"])
    # print(y["trace_embedding"])
    emb_x, emb_y = x["graph_embedding"][-1], y["graph_embedding"][-1]
    # print(emb_x)
    return cosine_similarity(emb_x, emb_y)


def graph_based_function(x, y):
    import networkx as nx
    if not x["not_degenerate"] or not y["not_degenerate"]:
        return 0
    # print(x["graph"])
    # print(y["graph"])
    g_x, g_y = x["graph"][-1], y["graph"][-1]
    if g_x is None or g_y is None:
        return 0
    distance = nx.graph_edit_distance(g_x, g_y)
    # print(distance)
...
def get_mbpp_selector(
    ...
    #################################################
    #################################################
    # our_function = lambda x, y: all(
    #     [
    #         trace_based_function(x, y, mbpp_good_execution_result),
    #         x["not_degenerate"],
    #         y["not_degenerate"],
    #     ]
    # )
    our_function = trace_based_function
    # our_function1 = graph_embedding_based_function
    ...
    # elif criterion == "our_ast_based":
    #     sample_selection_function = our_function1
    #     secondary_key_function = lambda x: x["sum_logprob"]
    # here
    # elif criterion=="our_method":
    #     sample_selection_function = our_function
    #     secondary_key_function = lambda x: x["sum_logprob"]
    elif criterion in [
        "sum_logprob",
...
def bootstrap_mbpp(args):
    '''
    通过!多次采样和评估(bootstrap_num)!，使用不同的样本选择策略，得到了在不同采样数量下的平均结果和标准差。
    '''
...
if __name__ == "__main__":
    ...
    args.criteria = ['random']  # all_crits
    ...
    if args.dataset == "spider":
        ...
        args.criteria = args.criteria + ["oracle", "our_trace_based"]  # , "our_ast_based", "mbr_exec"
        args.data_split = "test"
        acc_dict, std_dict = bootstrap_mbpp(args)
    elif "humaneval" in args.dataset or args.dataset == "mbpp_sanitized" or args.dataset == "mbpp_hyx":
        args.criteria = args.criteria + ["oracle", "our_trace_based"]  # , "our_ast_based", "mbr_exec"
        args.data_split = "test"
